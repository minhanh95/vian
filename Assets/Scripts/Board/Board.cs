...
public class Board
{
    ...
    internal void FillGapsWithNewItems()
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                NormalItem item = new NormalItem();
                Dictionary<NormalItem.eNormalType, int> dictItemCount = GetItemAmountEachType();
                NormalItem.eNormalType type = Utils.GetRandomNormalByNewRule(GetIgnorType(cell),dictItemCount);
    ...
    private NormalItem.eNormalType[] GetIgnorType(Cell cell)
    {
        List<NormalItem.eNormalType> ignorType = new List<NormalItem.eNormalType>();

        // Check if cell has a neighbor above
        if (cell.NeighbourUp != null && !cell.NeighbourUp.IsEmpty)
        {
            NormalItem it = cell.NeighbourUp.Item as NormalItem;
            if (it != null)
            {
                ignorType.Add(it.ItemType);
            }
        }

        // Check if cell has a neighbor below
        if (cell.NeighbourBottom != null && !cell.NeighbourBottom.IsEmpty)
        {
            NormalItem it = cell.NeighbourBottom.Item as NormalItem;
            if (it != null)
            {
                ignorType.Add(it.ItemType);
            }
        ...
        // Check if cell has a neighbor to the left
        if (cell.NeighbourLeft != null && !cell.NeighbourLeft.IsEmpty)
        {
            NormalItem it = cell.NeighbourLeft.Item as NormalItem;
            if (it != null)
            {
                ignorType.Add(it.ItemType);
            }
        ...
        // Check if cell has a neighbor to the right
        if (cell.NeighbourRight != null && !cell.NeighbourRight.IsEmpty)
        {
            NormalItem it = cell.NeighbourRight.Item as NormalItem;
            if (it != null)
            {
                ignorType.Add(it.ItemType);
            }
        }
        NormalItem.eNormalType[] array = ignorType.ToArray();
        return array;
    }
    private Dictionary<NormalItem.eNormalType, int> GetItemAmountEachType()
    {
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                Cell cell = m_cells[x, y];
                if (cell.IsEmpty) continue;
                NormalItem it = cell.Item as NormalItem;
                if (it != null)
                {
                    if (itemCounts.ContainsKey(it.ItemType))
                    {
                        itemCounts[it.ItemType]++;
                    }
                    else
                    {
                        itemCounts[it.ItemType] = 1;
                    }
                }
            }
        }
