...
public class Item
{
    ...
    public virtual void SetView()
    {
        ...
        {
            var item = PoolManager.Instance.GetObjectFromPool();
            View = item.transform;
            item.GetComponent<ItemViewController>().SetItemView(prefabname);           
        }
    }
    ...
    internal virtual void ExplodeView()
    {
        if (View)
        {
            View.DOScale(0.1f, 0.1f).OnComplete(
                () =>
                {
                    View.gameObject.GetComponent<PoolableObject>().ReturnToPool();
                    View = null;
                }
                );
        }
    ...
    internal void Clear()
    {
        ...
        if (View)
        {
            View.transform.GetComponent<PoolableObject>().ReturnToPool();
            View = null;
        }
    }
}
